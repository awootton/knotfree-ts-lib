...
export const defaultConnectInfo = {
    ...
    onConnect: () => { },
    onDisconnect: (err) => { }, // unused ?
    onMessage: (msg) => { console.log("override msg please", msg.toString()); },
    verbose: false,
    verboseRaw: false,
    write: (msg) => { }
};
export function NewDefaultRestarter() {
    let r = {
        connectInfo: {
            ...defaultConnectInfo,
        },
        ...
        onDisconnect: (r, err) => { },
        onMessage: (r, msg) => { console.log("Restarter onMessage", msg.toString()); },
        dontReconnect: false
    };
    r.connectInfo.onConnect = () => {
        r.onConnect(r);
    };
    r.connectInfo.onMessage = (msg) => {
        r.onMessage(r, msg);
    };
    r.connectInfo.onDisconnect = (err) => {
...
export function StartRestarter(restarter) {
    Startup(restarter.connectInfo);
}
export function NewDefaultPacketizer() {
    let packer = {
        restarter: NewDefaultRestarter(),
        onConnect: onConnectFunction,
        onPacket: (packer, u) => { console.log("override msg please"); },
        write: (p) => { console.log("override write please", p.toString()); },
        token: "",
        subs: [],
    ...
    packer.restarter.onMessage = (r, msg) => {
    ...
    let connected = false;
    function onConnectFunction(packer) {
        connected = true;
    ...
    function doSubscriptionsFunction(packer) {
        if (!connected) {
            return;
        }
        let i = 1;
...
export function Startup(params) {
    ...
    params.private_client_not_for_use.on('close', (b) => {
    ...
    params.private_client_not_for_use.on('data', (msg) => {
