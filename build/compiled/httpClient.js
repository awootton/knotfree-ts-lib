...
export const startHttpProxy = (config, host, port) => {
    ...
    function haveNewConfig(newConfig) {
...
export function NewDefaultHttpMonger() {
    let http = {
    ...
    http.write = (b, from) => {
        console.log("NewDefaultHttpMonger write", b.toString(), from.toBackingUniversal().toString());
...
function readChunkLength(buffer, pos) {
    let aLen = 0;
    let pops = pos;
...
export function NewDefaultReplyMonger(httpMonger, send) {
    let monger = {
        connectInfo: {
            ...client.defaultConnectInfo,
        },
        onMessage: (monger, msg) => { console.log("override msg please", msg.toString()); },
        httpMonger: httpMonger,
        send: send
    };
    var buffer = Buffer.alloc(0);
    monger.connectInfo.onMessage = (msg) => {
